...
export function TeamRosterManager({ team, season, onClose }: TeamRosterManagerProps) {
    ...
    const handleRemovePlayer = async (item: typeof roster[0]) => {
        const isSecondary = item.assignment_type === 'secondary' || item.isSecondary

        const confirmed = await confirm({
            title: isSecondary ? 'Quitar Doblaje' : 'Quitar Jugadora del Equipo',
            message: isSecondary
                ? 'La jugadora será desasignada como doblaje de este equipo. No se eliminarán sus datos.'
                : 'La jugadora será desasignada de este equipo. No se eliminarán sus datos ni historial.',
            severity: 'warning',
            confirmText: 'QUITAR'
        })
        ...
        try {
            if (isSecondary) {
                // For secondary assignments, use the secondary_id or id
                const secondaryId = item.secondary_id || item.id
                await playerTeamSeasonService.removeSecondaryAssignment(secondaryId)
                toast.success('Doblaje eliminado del equipo')
            } else {
                await playerTeamSeasonService.removePlayerFromTeamSeason(item.id)
                                            ...
                                            {canEditAllFields && (
                                                <td className="px-6 py-2.5 whitespace-nowrap text-right">
                                                    <button
                                                        ...
                                                        title={item.isSecondary ? "Quitar doblaje" : "Quitar del equipo"}
                                                    >
