...
import { X, Calendar, Clock, MapPin, ChevronRight, ChevronLeft, ChevronDown } from 'lucide-react'
...
function formatTimeForDisplay(time24h: string): string {
  // Input is already in 24h format (HH:MM), just return it
  return time24h
}
...
export function MatchWizard({ isOpen, onClose, initialStep = 1, matchId }: MatchWizardProps) {
  ...
  const [data, setData] = useState<WizardData>(() => {
    // If we have a matchId, load existing match data
    if (matchId) {
      ...
      if (existingMatch) {
        return {
          ...
          selectedPlayers: existingMatch.players.map(player => player.playerId) || [],
          sacadorInicialSet1: existingMatch.sacadorInicialSet1,
        }
      }
    }

    // Default values for new match
    ...
    return {
      ...
      teamSide: 'local',
      selectedPlayers: initialTeam?.players.map(player => player.id) || [],
  ...
  useEffect(() => {
    if (!isOpen) {
      // Reset display values and errors when modal closes
      setDisplayDate(formatDateForDisplay(data.date))
      setDisplayTime(formatTimeForDisplay(data.time))
      setDateError('')
      setTimeError('')
  ...
  }, [isOpen, data.date, data.time])

  const totalSteps = 3

  const handleNext = () => {
    if (currentStep < totalSteps) {
      // Validate step 1 has both team and teamSide selected
      if (currentStep === 1) {
        if (!data.teamId) {
          setError('Debes seleccionar tu equipo para este partido.')
          return
        }
        if (!data.teamSide) {
          setError('Debes seleccionar si tu equipo juega como local o visitante')
          return
        }
      }
      // Validate step 2 has required match information
      if (currentStep === 2) {
        if (!data.opponent.trim()) {
          setError('Debes ingresar el nombre del equipo contrario')
          return
        }
        if (!data.location.trim()) {
          setError('Debes ingresar la ubicación del partido')
          return
        }
        if (dateError) {
          setError(dateError)
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setData({ ...data, date: isoDate })
  }

  // Handle European time input
  const handleTimeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validateEuropeanTime(value)) {
      setTimeError('Formato de hora inválido (HH:MM, 24h)')
      return
    }
  ...
  const handleServeSelection = (weServeFirst: boolean) => {
    ...
    // Convertir 'visitante' a 'visitor' para el store
    const startingServerForStore = startingServer === 'visitante' ? 'visitor' : startingServer
    setData({ ...data, sacadorInicialSet1: startingServerForStore })
  }
  ...
  const handleCreateMatch = () => {
    ...
    try {
      ...
      if (!data.opponent.trim()) {
        throw new Error('Debes ingresar el nombre del equipo contrario')
      }

      if (!data.location.trim()) {
        throw new Error('Debes ingresar la ubicación del partido')
      }

      if (data.selectedPlayers.length === 0) {
        throw new Error('Debes seleccionar al menos una jugadora')
      }

      const matchPlayers = selectedTeam.players
        .filter(player => data.selectedPlayers.includes(player.id))
        .map(player => ({
          playerId: player.id,
          name: player.name,
          number: parseInt(player.number.toString()),
          position: player.role || 'L',
          starter: false, // No starters selected during creation
          stats: {
            serves: 0,
            aces: 0,
            serveErrors: 0,
      ...
      setData({
        ...
        teamId: teams[0]?.id || '',
        teamSide: 'local',
        selectedPlayers: teams[0]?.players.map(player => player.id) || [],
        sacadorInicialSet1: null,
      })
      setDisplayDate(formatDateForDisplay(newDate))
      setDisplayTime(formatTimeForDisplay(newTime))
      setDateError('')
      setTimeError('')
      setError('')

      // Close the modal and navigate to the new match detail
      onClose()
      navigate(`/matches/${newMatch.id}`)

    } catch (error) {
      setError(error instanceof Error ? error.message : 'Ha ocurrido un error al crear el partido. Inténtalo de nuevo.')
    }
  }
  ...
  const selectAllPlayers = () => {
    ...
    if (selectedTeam) {
      setData(prev => ({
        ...prev,
        selectedPlayers: selectedTeam.players.map(player => player.id)
      }))
    }
  }
  ...
  // Libero validation logic
  const getLiberoCount = (): number => {
    const selectedTeam = teams.find(team => team.id === data.teamId)
    if (!selectedTeam) return 0

    const selectedLiberos = selectedTeam.players.filter(
      player => data.selectedPlayers.includes(player.id) && player.role === 'L'
    )
    return selectedLiberos.length
  }

  const hasExcessLiberos = (): boolean => {
    return getLiberoCount() >= 3
  }
        ...
        <div className="modal-body overflow-y-auto flex-1">
          ...
          {error && (
            <div className="mb-6 bg-danger-50 border border-danger-200 rounded-lg p-4 flex items-start gap-3">
              <div className="w-1 h-1 bg-danger-500 rounded-full mt-2"></div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div
                    onClick={() => handleTeamSideChange('local')}
                    className={`p-4 border-2 rounded-xl cursor-pointer transition-all text-center relative overflow-hidden group ${data.teamSide === 'local'
                  ...
                  >
                    <div className="relative z-10">
                      <div className="flex items-center justify-center mb-2">
                        <div className={`w-5 h-5 rounded-full border-2 mr-2 flex items-center justify-center transition-colors ${data.teamSide === 'local'
                  ...
                  <div
                    onClick={() => handleTeamSideChange('visitante')}
                    className={`p-4 border-2 rounded-xl cursor-pointer transition-all text-center relative overflow-hidden group ${data.teamSide === 'visitante'
                      ? 'border-primary-500 bg-primary-50'
                      : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                      }`}
                  >
                    <div className="relative z-10">
                      <div className="flex items-center justify-center mb-2">
                        <div className={`w-5 h-5 rounded-full border-2 mr-2 flex items-center justify-center transition-colors ${data.teamSide === 'visitante'
                          ? 'border-primary-600 bg-primary-600'
                          : 'border-gray-300 group-hover:border-gray-400'
                          }`}>
                          {data.teamSide === 'visitante' && (
                            <div className="w-2 h-2 bg-white rounded-full" />
                          )}
                        </div>
                        <span className="font-medium text-gray-600">Tu equipo es:</span>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={data.opponent}
                    onChange={(e) => setData({ ...data, opponent: e.target.value })}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className={`input-field w-full ${dateError ? 'border-danger-500 focus:border-danger-500 focus:ring-danger-500' : ''}`}
                    />
                    {dateError && (
                      <p className="text-danger-500 text-xs mt-1 font-medium">{dateError}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="HH:MM"
                      className={`input-field w-full ${timeError ? 'border-danger-500 focus:border-danger-500 focus:ring-danger-500' : ''}`}
                    />
                    {timeError && (
                      <p className="text-danger-500 text-xs mt-1 font-medium">{timeError}</p>
                    )}
                  </div>
                </div>
          ...
          {currentStep === 3 && selectedTeam && (
            <div className="space-y-6">
              {/* Serve Selection for Set 1 */}
              <div className="space-y-3">
                <h3 className="text-lg font-semibold text-gray-900">¿Quién saca primero?</h3>
                <div className="space-y-3">
                  <button
                    onClick={() => handleServeSelection(true)}
                    className={`w-full py-3 px-4 rounded-xl font-bold transition-all ${data.sacadorInicialSet1 === (data.teamSide === 'local' ? 'local' : 'visitor')
                  ...
                  <button
                    ...
                    className={`w-full py-3 px-4 rounded-xl font-bold transition-all ${data.sacadorInicialSet1 === (data.teamSide === 'local' ? 'visitor' : 'local')
                      ? 'bg-primary-600 text-white shadow-md transform scale-[1.02]'
                      : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      }`}
                  >
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                  {selectedTeam.players.map((player) => (
                    <div
                      key={player.id}
                      className={`p-2 border-2 rounded-xl cursor-pointer transition-all flex flex-col items-center justify-center gap-1 relative group min-h-[110px] ${data.selectedPlayers.includes(player.id)
                        ? 'border-primary-500 bg-primary-50 shadow-sm'
                        : 'border-gray-200 hover:border-primary-300 hover:bg-gray-50'
                        }`}
                      onClick={() => togglePlayerSelection(player.id)}
                      title={`Seleccionar ${player.name}`}
                    >
                      {/* Player Number Circle */}
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-sm border-2 border-white mb-1.5 transition-colors flex-shrink-0 ${data.selectedPlayers.includes(player.id) ? 'bg-primary-600' : 'bg-gray-900'}`}>
                        {player.number}
                      </div>

                      {/* Position Badge */}
                      <span title={POSITION_NAMES[player.role || 'L']} className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase mb-1 ${player.role === 'L' ? 'bg-yellow-400 text-yellow-900' :
                          player.role === 'S' ? 'bg-blue-100 text-blue-800' :
                            player.role === 'OH' ? 'bg-green-100 text-green-800' :
                              player.role === 'MB' ? 'bg-purple-100 text-purple-800' :
                      ...
                      {/* Name */}
                      <p className="font-bold text-gray-900 text-xs truncate w-full text-center">{player.name.split(' ')[0]}</p>

                      {data.selectedPlayers.includes(player.id) && (
                ...
                {data.selectedPlayers.length === 0 && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    <p className="text-sm text-red-800">
                      Debes seleccionar al menos una jugadora para este partido
                    </p>
                  </div>
                )}

                {/* Libero validation warning */}
                {hasExcessLiberos() && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-red-800">
                          Demasiadas líberos convocadas
                        </p>
                        <p className="text-sm text-red-700 mt-1">
                          Has seleccionado {getLiberoCount()} líberos. Solo se pueden convocar 2 líberos por partido.
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="modal-footer">
            ...
            <div className="flex gap-3">
              ...
              {currentStep < totalSteps ? (
                <button
                  onClick={handleNext}
                  disabled={(currentStep === 1 && (!data.teamId || !data.teamSide)) || (currentStep === 2 && (!data.opponent.trim() || !data.location.trim() || !!dateError || !!timeError))}
                ...
                <button
                  onClick={() => {
                    if (hasExcessLiberos()) {
                      setShowLiberoValidationModal(true)
                    } else {
                      handleCreateMatch()
                    }
                  }}
                  disabled={data.selectedPlayers.length === 0 || !data.sacadorInicialSet1}
